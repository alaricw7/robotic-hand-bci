"""
data.py - 数据加载、预处理、LOSO 划分。

本 loso_npz 版本优先读取 data/processed/pythondata1 这种目录结构:
    {data_root}/S1/S1_EEGNet_NoICA_uV.npz
NPZ 中需要包含 X=(trials, channels, samples), y=(trials,)。
"""
...
def _split_train_val_time_ordered(indices, y, cfg):
    """
    按类别分别使用原始 trial 顺序切 validation。

    LOSO validation 依赖输入 NPZ 保留每个 subject 内的时间顺序；如果
    上游保存前打乱了 trial，"末尾 val_size" 就不再表示时间末段。
    """
...
def get_fold_loaders(X, y, fold_idx, cfg):
    ...
    split_info = {
        ...
        "split_policy": "outer stratified 10-fold test; stratified random val_size validation from train_val",
        "counts": {
...
def get_loso_loaders(train_subject_data, X_test, y_test, cfg):
    """
    为 LOSO 构造 train/val/test DataLoader。

    test 是被试级别留出；validation 只从训练被试的数据中切分。
    每个训练 subject 先按类别切 train/val，再只用该 subject 的 train
    部分拟合 standardizer 并应用到该 subject 的 train/val。
    held-out test subject 使用自己的全部 trial 无监督拟合 standardizer
    并应用到自身，不使用标签。
    """
    train_X_parts, train_y_parts = [], []
    val_X_parts, val_y_parts = [], []
    per_subject_splits = {}
    standardizer_info = None
    if cfg.channel_normalize:
        standardizer_info = {
            "mode": "per_subject_train_portion",
            "train_subjects": {},
            "test_subject": {},
        }

    for subject_id, X_sid, y_sid in train_subject_data:
        X_sid = np.asarray(X_sid)
        y_sid = np.asarray(y_sid, dtype=np.int64)

        subject_indices = np.arange(len(y_sid), dtype=np.int64)
        train_idx_sid, val_idx_sid = _split_train_val_time_ordered(subject_indices, y_sid, cfg)

        X_train_sid_raw = X_sid[train_idx_sid]
        X_val_sid_raw = X_sid[val_idx_sid]
        y_train_sid = y_sid[train_idx_sid]
        y_val_sid = y_sid[val_idx_sid]

        if cfg.channel_normalize:
            mean_sid, std_sid = fit_channel_standardizer(X_train_sid_raw)
            X_train_sid = apply_channel_standardize(X_train_sid_raw, mean_sid, std_sid)
            X_val_sid = apply_channel_standardize(X_val_sid_raw, mean_sid, std_sid)
            standardizer_info["train_subjects"][int(subject_id)] = {
                "fit_on": "subject_train_portion",
                "mean_shape": list(mean_sid.shape),
                "std_shape": list(std_sid.shape),
            }
        else:
            X_train_sid = X_train_sid_raw
            X_val_sid = X_val_sid_raw

        train_X_parts.append(X_train_sid)
        train_y_parts.append(y_train_sid)
        val_X_parts.append(X_val_sid)
        val_y_parts.append(y_val_sid)

        per_subject_splits[int(subject_id)] = {
            "train_idx": train_idx_sid.tolist(),
            "val_idx": val_idx_sid.tolist(),
            "counts": {
                "train": int(len(train_idx_sid)),
                "val": int(len(val_idx_sid)),
            },
            "class_counts": {
                "train": _label_counts(y_train_sid, cfg.n_classes),
                "val": _label_counts(y_val_sid, cfg.n_classes),
            },
        }
    ...
    if cfg.channel_normalize:
        mean_test, std_test = fit_channel_standardizer(X_test)
        X_test = apply_channel_standardize(X_test, mean_test, std_test)
        standardizer_info["test_subject"] = {
            "fit_on": "heldout_subject_all_trials_unsupervised",
            "mean_shape": list(mean_test.shape),
            "std_shape": list(std_test.shape),
        }

    train_loader = _build_loader(X_train, y_train, cfg, shuffle=True, drop_last=True)
    val_loader = _build_loader(X_val, y_val, cfg, shuffle=False)
    test_loader = _build_loader(X_test, y_test, cfg, shuffle=False)

    split_info = {
        "train_idx": "per_subject_local_indices",
        "val_idx": "per_subject_local_indices",
        "test_idx": "heldout_subject_trials",
        "split_policy": "LOSO test subject; each train subject contributes per-class time-ordered last val_size validation",
        "per_subject_splits": per_subject_splits,
        "counts": {
        ...
        "class_counts": {
            "train": _label_counts(y_train, cfg.n_classes),
            "val": _label_counts(y_val, cfg.n_classes),
            "test": _label_counts(y_test, cfg.n_classes),
        },
        "standardizer": standardizer_info,
    }
